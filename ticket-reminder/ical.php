...
if ( $_GET['sj'] == 1 ) {
	...
	$dateend 		= $datestart + 30 * 60;
	$filename 	= $_GET['filename'];
} else {
	exit;
}
//echo dateToCal($datestart);exit;
...
BEGIN:VCALENDAR
VERSION:2.0
PRODID:-//psu.se//iCal Maker
BEGIN:VEVENT
DTSTAMP:<?= dateToCal(time()) ?>Z
UID:<?= uniqid() ?>@psu.se
DTSTART:<?= dateToCal($datestart) ?>Z
DTEND:<?= dateToCal($dateend) ?>Z
